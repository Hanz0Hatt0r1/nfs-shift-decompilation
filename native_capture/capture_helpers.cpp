...
std::vector<unsigned char> copy_declaration(const void* raw_declaration) {
    const auto* declaration = static_cast<const D3DVERTEXELEMENT9*>(raw_declaration);
...
std::vector<unsigned char> copy_shader(const void* raw_shader) {
    const auto* shader = static_cast<const DWORD*>(raw_shader);
    std::vector<unsigned char> bytes;
