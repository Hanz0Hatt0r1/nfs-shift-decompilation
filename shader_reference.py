...
def validate_vertex_program_inputs(program: ShaderProgram) -> dict[str, Any]:
    """Validate the vertex inputs the deterministic reference renderer can source."""
    unsupported = []
    for item in program.inputs:
        usage = str(item.get("usage") or "").upper()
        index = int(item.get("index", 0))
        if usage == "POSITION" and index == 0:
            continue
        if usage == "TEXCOORD" and 0 <= index <= 4:
            continue
        unsupported.append({
            "usage": usage,
            "index": index,
...
def material_constants_from_payload(
    ...
    """Convert a ready SHIFT.MaterialConstantPayload/1 into a D3D9 c-bank."""
    binding = payload or {}
