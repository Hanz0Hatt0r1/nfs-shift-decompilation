...
def test_render_command_emits_concrete_gles_vertex_attribute_setup():
    packet = _packet()
    ...
    assert setup["200"]["integer_pointer"] is False
    ...
    assert setup["460"]["integer_pointer"] is False
    assert setup["460"]["channel_order_candidates"] == ["RGBA", "BGRA"]


def test_render_command_blocks_unknown_vertex_storage():
    packet = _packet()
...
def test_render_command_rejects_normalized_integer_bone_indices():
    packet = _packet()
    packet["mesh"]["vertex_layout"]["attributes"] = [{
        "property_id": "580",
        "location": 2,
