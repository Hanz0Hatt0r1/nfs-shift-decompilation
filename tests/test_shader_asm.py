...
def test_glass_programs_have_register_operands():
    ...
    for b in blobs:
        ...
        assert p.instructions[0].name in ('DCL','DEF','DEFI','MOV')
...
def _program_with(opcode: int, name: str):
    ...
    srcs = [Operand(token=0x80000000+i, kind='source', reg_type=0, index=i, swizzle='xyzw') for i in (1,2,3)]
    return ShaderProgram(
        offset=0, end=0, stage='pixel', major=3, minor=0,
        instructions=[Instruction(0, opcode, name, 0, 4, 0, False, [dst, *srcs])],
        inputs=[], outputs=[], samplers=[], constants=[], temps=[0,1,2,3], unsupported_opcodes=[]
    )


def test_glsl_cmp_lrp_keep_d3d9_operand_semantics():
