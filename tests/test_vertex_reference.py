...
from vertex_reference import (
    build_vertex_inputs,
    execute_vertex,
    execute_vertex_program,
)
...
def test_build_vertex_inputs_maps_position_and_texcoord0():
    inputs = build_vertex_inputs(_vertex_program(), _mesh(), 1)
    assert inputs == {
        0: (4.0, 5.0, 6.0, 1.0),
        1: (0.3, 0.4, 0.0, 1.0),
    }
...
def test_execute_vertex_preserves_typed_oR0_and_oT0_outputs():
    result = execute_vertex(_vertex_program(), _mesh(), 0)
...
def test_execute_vertex_requires_declared_attribute():
    mesh = _mesh()
    mesh["uv_layers"] = {}
    with pytest.raises(ValueError, match="requires TEXCOORD0"):
        execute_vertex(_vertex_program(), mesh, 0)


def test_execute_vertex_program_runs_all_mesh_vertices():
    result = execute_vertex_program(_vertex_program(), _mesh())
    assert result["status"] == "executed"
