...
Current `main` is at Phase 224. The Python, native and Windows D3D9 capture-producer CI paths are green on the last completed baseline; the current work strengthens runtime same-instance proof without requiring a capture to exist in CI.
...
The supplied SHIFT.exe PE tables can now be normalized into SHIFT.D3D9PESemanticMap/1. A separate runtime parity validator compares declaration Type/Usage numeric fields against that exact executable ABI. Numeric parity deliberately does not assign MEB property identity; same-instance declaration/draw correlation remains separate.
