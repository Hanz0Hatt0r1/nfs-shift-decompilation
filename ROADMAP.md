...
`SHIFT.D3D9DrawStateSnapshot/1` now defines the exact state captured at each
`DrawIndexedPrimitive` boundary. The snapshot keeps forensic binding history while
also exposing normalized active stream and texture bindings plus latest constant
register state.

`d3d9_draw_snapshot_schema.py` validates generated snapshots during runtime report
construction. Invalid snapshots cannot become same-instance proof candidates.
Downstream BMW runtime consumers therefore share one explicit draw-state contract
instead of independently interpreting frame-level event history.
...
`SHIFT.BMWRuntimeGoldenGate/1` now requires `runtime.same_instance_gate.ready == true` in addition to shader/resource/declaration/input/draw/constant parity. This prevents a runtime golden result from accepting a descriptor match found in an unrelated created declaration.

Next: run the strict runtime tracer on an authentic retail capture for the exact BMW M3 MEB. This is the primary remaining external proof gap.
...
\n## Phase 153: first real MEB render\n\nThe repository now has a thin `BFF -> XMem/LZX -> MEB -> reference_renderer -> PPM` adapter. A supplied BMW M3 body MEB can be selected by its exact archive path, rendered as geometry-only flat gray, and optionally emitted as neutral mesh JSON with full decoded-resource provenance.\n\nThis closes the first real-data rasterization boundary without claiming final BMW material shading or resolving the still-evidence-sensitive COLOR0/COLOR1 ABI. The next rendering step is the same real primitive routed through BMT -> FX -> FXO selection, texture DDS payloads, material constants and the selected VS -> PS reference program.\n\n## Phase 154: VHF-driven whole vehicle geometry render\n\nThe first whole-vehicle geometry boundary is now implemented for the supplied BMW M3 VHF. The adapter selects non-damage LODA object nodes, resolves VHF parented matrices, decodes each exact MEB from the same BFF, transforms the positions into world space and submits one combined neutral mesh to the existing desktop reference rasterizer.\n\nThe default KIT00 profile includes the base kit plus shared wheels, tires, discs, calipers, mirrors and lightglow objects while excluding alternative kits and damage meshes. The output records per-part MEB SHA-256 and world-matrix provenance.\n\nThe next blocker is still material execution: the real BMT points at `render/shaders/bodywork.fx`, while the supplied BMW archive carries the cached FXO permutations but not that source file. A full paint render therefore requires the corresponding render/shader source archive or an equivalent evidenced source payload.\n
...
## Phase 218: native Vulkan BMW bundle runner

A native `shift_vulkan_bundle_runner` target now consumes the prepared BMW Vulkan
bundle artifacts without parsing BFF/MEB/RenderCommand JSON. The runner validates and
loads geometry/constants/resource packets.
...
The bundle interface validator now verifies descriptor stage compatibility with the
...
Final `BMWMaterialSliceVulkan/1` readiness is now derived from the final merged blocker set. Synthetic DXT1 DDS fixtures encode the standard 124-byte DDS header fields at the correct caps/caps2 offsets.
...
## Phase 338: BMW texture snapshot coverage

The native D3D9 capture producer now defaults texture snapshot capture to all BMW
paint stages s0..s4. This fixes the previous omission of the material-owned s1
diffuse and s2 specular surfaces, allowing Phase 337 content parity to consume
complete material texture snapshots without requiring a hidden environment
override.
...
\n\n## Phase 345: reproducible BMW runtime buffer artifacts\n\nAdded `bmw_meb_runtime_buffer_artifacts.py` to reproduce the byte candidates\nconsumed by Phase 344 directly from the retail M3 BFF. The producer preserves\nraw MEB property payload bytes, interleaves them in deterministic MEB property\norder, emits the complete little-endian INDEX16 stream and one binary per\nprimitive, and records SHA-256/size provenance in a manifest.\n\nFor the BMW body the candidate vertex buffer is 3,550 vertices × 76 bytes =\n269,800 bytes. The six primitive INDEX16 candidates are 300, 12,588, 14,772,\n1,224, 1,152 and 168 bytes. These are still reconstructed candidates: exact\nD3D9 runtime identity remains gated by the Phase 344 Lock/Unlock payload capture.\n