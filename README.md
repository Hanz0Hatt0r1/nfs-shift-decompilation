...
После exact runtime selection можно построить fail-closed execution input:
...
Контракт сохраняет selected VS/PS identity, indexed draw ranges, D3D9 constant banks и runtime texture-stage bindings. s0/s3 остаются runtime-external до тех пор, пока их реальные resource identities и image payloads не будут доказаны.
...
Selector сравнивает runtime `ShaderPermutationIdentity/1` с полным FXO candidate set. При наличии exact identity использует `identity_sha256`, при отсутствии — exact VS/PS pair bytes. Одновременно требует совпадения MEB resource identity; ties остаются `ambiguous`.
...
Для реального BMW material path, где MEB/BMT/textures лежат в vehicle archive, а `bodywork.fx` и FXO cache — в `RENDER.bff`:
    ...
    python bmw_split_bff_shader_probe.py \\
      BMW_M3_E36.bff out/bmw_m3_e36_shader_probe.json \\
      --supplemental-bff RENDER.bff

Probe сохраняет provenance по обоим архивам, shader reference из BMT, найденный `bodywork.fx`, число FXO-кандидатов, selected permutation identity и наличие linked VS/PS pair. Он ничего не подменяет и не выбирает runtime-global resources.
...
The M3 paint contract now validates the real material shape emitted by `compile_material()`, so the documented sampler/register chain can be checked directly on pipeline output rather than a separate fixture format.


## Phase 138: BMW paint contract enforcement

The real `compile_material()` path now validates the exact BMW M3 paint material against its evidence-backed sampler/shader contract and propagates failures into `StaticDraw/1`.
...
The M3 paint contract now accepts the structured specialization evidence emitted by the material linker, preserving the existing requested-feature provenance through compile and shader gates.
...
The project can now emit the existing `SHIFT.BMWMaterialSlice/1` directly from the real M3 BFF path, with a neutral MEB payload and ready `StaticDraw/1`/`RenderCommand/1` when all archived evidence gates pass.


## Phase 146: BMW M3 MEB evidence parity

The exact M3 body MEB is now preserved as a compact evidence snapshot and machine-compared with the golden manifest, including resource provenance, primitive ranges, descriptor triples and property layouts.
...
## Phase 149: runtime golden same-instance requirement

The unified BMW runtime golden gate now requires strict same-instance declaration proof, not merely a matching created declaration. A real retail capture is the remaining external dependency.


## Phase 150: same-frame runtime proof

Strict runtime instance proof now requires a real indexed draw in the same captured frame as the bound M3 declaration/resource match.
