...
> **Текущий статус:** mainline дошла до **Phase 172** — реальный BMW M3 уже проходит `VHF → MEB → BMT → DDS`, для `bodywork.fx` зафиксирован реальный FXO corpus, runtime selector умеет exact VS/PS match, D3D9 capture пишет shader/constants/SetTexture и resource descriptors, а `Present` может опционально сохранять реальный backbuffer в PPM для визуального checkpoint. Следующий визуальный gate — один runtime capture кадра M3 body.
