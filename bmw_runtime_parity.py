...
def _constant_requirements(material_slice: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in binding.get('bindings') or []:
        ...
        out.append({'name':row.get('name'),'stage':stage,'register_index':start,'register_count':count,'registers':list(range(start,start+count))})
    return out
...
def validate_runtime_parity(material_slice: Mapping[str, Any], runtime_report: Mapping[str, Any], *, usage_map: Mapping[int,int] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        'shader_join':join,
        'constant_parity':{'status':constant_status,'requirements':constant_rows,'checks':constant_checks},
        'declaration_parity':{'status':decl_status,'checks':declaration_checks,'declaration_ptr':declaration_ptr},
    }

def validate_files(material_path: str | Path, runtime_path: str | Path, *, usage_map_path: str | Path | None = None) -> dict[str, Any]:
    ...
    return validate_runtime_parity(material,runtime,usage_map=usage_map)

def main() -> int:
    ap=argparse.ArgumentParser(description='Validate BMW runtime shader, constant and D3D9 declaration parity')
    ap.add_argument('material_slice'); ap.add_argument('runtime_report'); ap.add_argument('output'); ap.add_argument('--usage-map')
    args=ap.parse_args(); report=validate_files(args.material_slice,args.runtime_report,usage_map_path=args.usage_map)
