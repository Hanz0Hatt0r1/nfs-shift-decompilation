...
def analyze_d3d9_stream_topology(source: str | bytes) -> dict[str, Any]:
    ...
    inputs = {
        ...
        "usage_ordinal_array": {"status": "observed" if _any(text,
            ...
            "FUN_00853c40(local_5c)", "*(int *)(uVar3 + 0x74)",
            "FUN_00853c40(*(int *)(*(int *)(uVar3 + 0x74) + uVar11 * 4))", start=start) else "not-found"},
        "channel_array": {"status": "observed" if (
            _any(text,
                "*(int *)(local_28 + 0x48)", "*(int *)(*(int *)(local_28 + 0x48) + iVar12 * 4)",
                "*(undefined1 *)(*(int *)(local_28 + 0x48)", start=start)
            or _any(text,
                "*(int *)(uVar3 + 0x78)",
                "*(undefined1 *)(*(int *)(uVar3 + 0x78) + uVar11 * 4)", start=start)
        ) else "not-found"},
    }
