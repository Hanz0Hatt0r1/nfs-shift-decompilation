...
def _vec4(value: Iterable[float] | None, *, default_w: float = 0.0) -> tuple[float, float, float, float]:
...
def execute_vertex(
    ...
    *,
    ...
    try:
        ...
        execution = execute_shader_ir(program, inputs=inputs, constants=constants)
    except ValueError as exc:
        return {
            "format": FORMAT,
            "status": "error",
            "blocking_reasons": [f"vertex-input:{exc}"],
        }
    if execution.get("status") != "executed":
        return {"format": FORMAT, **execution}
