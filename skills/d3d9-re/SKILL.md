...
Build `native_capture` on an authorized Windows test installation with the
MSVC toolchain and run the resulting `d3d9.dll` beside the game executable.
Set `SHIFT_D3D9_CAPTURE` to the JSONL output path.

The producer covers declaration creation/bind, stream/index state, VS/PS
creation/bind, VS/PS float constants and indexed draws. Successful `Present`
advances the frame counter.

Treat the producer as evidence collection only. It must not infer
`resource_path`, `resource_sha256`, MEB Usage mappings or COLOR ABI.
Pass the JSONL through `validate-d3d9-capture` and then
