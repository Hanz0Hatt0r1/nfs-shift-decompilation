...
def select_runtime_shader(
    ...
    *,
    ...
    binding = material_input.get("material_binding")
    binding_rows = binding.get("bindings") if isinstance(binding, Mapping) else material_input.get("bindings")
    for row in binding_rows or []:
        ...
        try:
            ...
            continue
        sampler_type = str(row.get("sampler_type") or "").strip().lower()
    ...
    for frame in runtime_report.get("frames") or []:
        ...
        if require_same_resource and same_resource is not True:
            continue
        texture_ok, _missing_texture_stages = _texture_stage_status(frame, expected_external_stages)
